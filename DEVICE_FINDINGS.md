...
- Style-panel probe `style-panel-probe-20260722-g`, APK SHA-256 `75945f5c7d256fd68ab2a2eaa28b7d452c8e539d61313592039e48045f952b7b`, changes the surface to 240×180 so the 80×60 frame fills it by an exact 3× expansion. It draws diagnostics over the moving image and adds bounded left/right and bidirectional-dial selection across ten exact preset IDs. Style changes reuse the last decoded raw frame and reset temporal history; they add no queue or per-frame allocation. ASan/UBSan, ThreadSanitizer, legacy build, APK verification, and physical installation pass; launch and lifecycle results are pending.
...
1. Confirm that the style-panel image fills the complete 240×180 panel and that no static side letterbox remains.
2. Use left/right and both dial directions to traverse all ten named styles and wrap in both directions without freezing the live image.
3. Run the style-panel probe for 30 seconds and confirm processed FPS around 6–10, no decode error, and `OUT` staying at 0 or briefly 1.
4. Open and exit the style-panel probe five times without capture. After every exit, take one normal-camera photo and confirm writing completes normally.
5. Take one photo inside RetroLens, wait for its media LED to stop, exit, then take one normal-camera photo and confirm writing completes normally.
6. Verify autofocus, still capture, disabled Movie message, Delete/back exit, and application-menu responsiveness.
7. Stop immediately on `BUFFER IMBALANCE`, a frozen frame counter, persistent media LED, or `Writing memory`; reinstall the preserved thread or safety APK before further diagnosis.
