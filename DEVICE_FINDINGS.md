...
- Sequence probe `sequence-probe-20260722-e`, APK SHA-256 `000bc1d559b99c32768b598df19a469d4217ff76b5fdbdc03d6764e7ac8d3802`, reintroduces only CameraSequence acquisition. It starts after normal preview and the proven native panel, uses one 256 KiB direct buffer and one-frame Sony request, validates JPEG markers and length, releases every returned `DeviceMemory` in `finally`, and displays RX/release/outstanding/FPS/size metrics. It contains no frame dump, external-storage, decode, filter-runtime, derivative, or video path. Host ASan/UBSan and ThreadSanitizer tests, armeabi build, API-10 Java build, APK verification, and physical installation pass. Physical launch/lifecycle results are pending.
...
1. Run the sequence probe for 30 seconds and confirm `SEQUENCE ACTIVE`, analytical FPS near the earlier 9–10 FPS observation, JPEG sizes roughly 55–83 KiB, and `OUT` staying at 0 or briefly 1.
2. Open and exit the sequence probe five times without capture. After every exit, take one normal-camera photo and confirm writing completes normally.
