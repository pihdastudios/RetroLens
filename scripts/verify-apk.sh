...
grep -q "package: name='io.pihda.retrolens' versionCode='1' versionName='1.0.0'" <<< "$badging"
grep -q "launchable-activity: name='io.pihda.retrolens.RetroLensActivity'" <<< "$badging"
grep -q "label='RetroLens'" <<< "$badging"
grep -q "sdkVersion:'10'" <<< "$badging"
grep -q "targetSdkVersion:'10'" <<< "$badging"
grep -q "lib/armeabi/libretrolens.so" < <(unzip -Z1 "$apk")
if unzip -Z1 "$apk" | grep -q 'libgesture_detector'; then
    echo "Obsolete gesture detector library was packaged" >&2
    exit 1
fi
