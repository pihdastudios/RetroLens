...
static unsigned char readJpeg(unsigned char *destination,
                              unsigned char requested, unsigned char *actual,
                              void *context) {
  JpegInput *input = static_cast<JpegInput *>(context);
  if (!input || !destination || !actual)
    return PJPG_STREAM_READ_ERROR;
  size_t remaining =
      input->offset < input->length ? input->length - input->offset : 0;
...
static void makeDirectory(const char *path) {
  if (mkdir(path, 0775) != 0 && errno != EEXIST) {
    __android_log_print(ANDROID_LOG_ERROR, "RetroLens",
                        "mkdir failed path=%s errno=%d", path, errno);
  }
}
...
static uint16_t blend565(uint16_t base, uint16_t over, int alpha) {
  ...
  int orr = ((over >> 11) & 31) * 255 / 31, og = ((over >> 5) & 63) * 255 / 63,
      ob = (over & 31) * 255 / 31;
  return rgb565((br * (100 - alpha) + orr * alpha) / 100,
                (bg * (100 - alpha) + og * alpha) / 100,
...
public:
  Runtime(const char *root, const char *model, const char *version)
      : window_(0), running_(true), valid_(true), threadStarted_(false),
        recorderThreadStarted_(false), framePending_(false), frameInUse_(false),
        jpegLength_(0), frameTimestamp_(0), hasFrame_(false),
        hasPrevious_(false), selected_(0), intensity_(100),
        scene_(SCENE_CAMERA), controlsVisible_(true), compare_(false), touchActive_(false),
        favoriteMaskLo_(0), favoriteMaskHi_(0), frameCount_(0),
        droppedFrames_(0), decodeMs_(0), filterMs_(0), renderMs_(0), fps_(0),
        fpsFrames_(0), fpsStart_(monotonicMs()), startMs_(monotonicMs()),
        lastFrameMs_(0), lastTouchMs_(0), touchDownMs_(0), touchStartX_(0),
        touchStartY_(0), error_(false), captureFlashUntil_(0),
        saveRequest_(false), saveTimestamp_(0), savePreset_(0), saveIntensity_(100), recThreadRunning_(true),
        recCommand_(0), recPending_(false), recJob_(JOB_NONE), recPreset_(0), recIntensity_(100),
        recording_(false), recordingInterrupted_(false), recordStartMs_(0), recordEndMs_(0),
        lastRecordFrameMs_(0), recordPreset_(0), recordIntensity_(100), recordFrames_(0), recordDropped_(0),
    ...
    raw_ = (Pixel *)malloc(retrolens::kFrameWidth * retrolens::kFrameHeight *
                           sizeof(Pixel));
    output_ = (Pixel *)malloc(retrolens::kFrameWidth * retrolens::kFrameHeight *
                              sizeof(Pixel));
    scratch_ = (Pixel *)malloc(retrolens::kFrameWidth *
                               retrolens::kFrameHeight * sizeof(Pixel));
    previous_ = (Pixel *)malloc(retrolens::kFrameWidth *
                                retrolens::kFrameHeight * sizeof(Pixel));
    recorderFrame_ = (Pixel *)malloc(retrolens::kFrameWidth *
                                     retrolens::kFrameHeight * sizeof(Pixel));
    encoded_ = (unsigned char *)malloc(kEncodedCapacity);
    if (!input_ || !raw_ || !output_ || !scratch_ || !previous_ ||
    ...
    memset(output_, 0,
           retrolens::kFrameWidth * retrolens::kFrameHeight * sizeof(Pixel));
    loadSettings();
    log("runtime create version=%s model=%s presets=%d allocations=%lu",
        version_, model_, retrolens::presetCount(),
        (unsigned long)(kInputCapacity + kEncodedCapacity +
                        5 * retrolens::kFrameWidth * retrolens::kFrameHeight *
                            sizeof(Pixel)));
  ...
  void captureRequested(int64_t now) { captureFlashUntil_ = now + 180; }
...
private:
  ...
  int frameCount_, droppedFrames_, decodeMs_, filterMs_, renderMs_, fps_,
      fpsFrames_;
  ...
  bool settingsDirty_;
  char root_[512], base_[576], model_[64], version_[32], logPath_[640],
      settingsPath_[640];
  FILE *logFile_;
  ...
  void setupPaths() {
    ...
    makeDirectory(base_);
    const char *names[] = {"CONFIG", "PRESETS", "PHOTOS", "CLIPS",
                           "THUMBNAILS"};
    for (int i = 0; i < 5; i++) {
      char p[640];
    ...
    snprintf(settingsPath_, sizeof(settingsPath_), "%s/CONFIG/settings.cfg",
             base_);
  }
  void loadSettings() {
  ...
  void saveSettingsNow() {
    ...
    fprintf(f,
            "version=1\npreset=%s\nintensity=%d\nfavorites_lo=%llx\nfavorites_"
            "hi=%llx\nmotion=full\nquality=performance\nrecord_fps=10\n",
            retrolens::presetAt(selected_).id, intensity_,
            (unsigned long long)favoriteMaskLo_,
  ...
  void changePreset(int delta) {
    ...
    selected_ = (selected_ + delta + retrolens::presetCount()) %
  ...
  bool isFavorite() const {
    return selected_ < 64
               ? (favoriteMaskLo_ & ((uint64_t)1 << selected_)) != 0
  ...
  bool decodeReduced(const unsigned char *jpeg, int length) {
    ...
    if (status || info.m_width != 640 || info.m_height != 480) {
      log("JPEG decode init failed status=%u dimensions=%dx%d",
          (unsigned)status, info.m_width, info.m_height);
      return false;
    }
    for (int my = 0; my < info.m_MCUSPerCol; my++)
      for (int mx = 0; mx < info.m_MCUSPerRow; mx++) {
        ...
        int orr = (right * retrolens::kFrameWidth + info.m_width - 1) /
                  info.m_width,
            ob = (bottom * retrolens::kFrameHeight + info.m_height - 1) /
                 info.m_height;
        for (int y = ot; y < ob; y++) {
    ...
    while (true) {
      ...
      if (frame) {
        ...
        if (decoded) {
          ...
          retrolens::processFrame(
              raw_, output_, scratch_, hasPrevious_ ? previous_ : 0,
              retrolens::kFrameWidth, retrolens::kFrameHeight, p, intensity_,
              0x52455452U, timestamp);
          memcpy(previous_, output_,
                 retrolens::kFrameWidth * retrolens::kFrameHeight *
                     sizeof(Pixel));
          hasPrevious_ = true;
          hasFrame_ = true;
          error_ = false;
  ...
  void recorderLoop() {
    while (true) {
      pthread_mutex_lock(&recMutex_);
      while (recThreadRunning_ && !recCommand_ && !recPending_ &&
             !settingsDirty_)
  ...
  void timestampName(char *out, size_t size, int64_t stamp) {
    ...
    snprintf(out, size, "%04d%02d%02d_%02d%02d%02d", local.tm_year + 1900,
  ...
  void startRecording() {
    ...
    snprintf(tempPath_, sizeof(tempPath_), "%s/CLIPS/%s_%s.avi.tmp", base_,
             name, p.id);
    snprintf(finalPath_, sizeof(finalPath_), "%s/CLIPS/%s_%s.avi", base_, name,
             p.id);
    snprintf(manifestPath_, sizeof(manifestPath_), "%s/CLIPS/%s_%s.json", base_,
             name, p.id);
    recording_ = avi_.open(tempPath_, retrolens::kFrameWidth,
                           retrolens::kFrameHeight, 10);
  ...
  void finishRecording(bool interrupted) {
    ...
    if (f) {
      ...
      fprintf(f,
              ...
              recordFrames_, recordDropped_, interrupted ? "true" : "false",
              recordIntensity_, (long long)recordStartMs_, (long long)recordEndMs_);
  ...
  void writeJob(int job) {
    ...
    bool ok = retrolens::encodeJpeg(recorderFrame_, retrolens::kFrameWidth,
                                    retrolens::kFrameHeight, 82, encoded_,
                                    kEncodedCapacity, &size);
    if (ok && job == JOB_VIDEO) {
      ...
      snprintf(tmp, sizeof(tmp), "%s/PHOTOS/%s_%s_preview.jpg.tmp", base_, name,
               p.id);
      snprintf(path, sizeof(path), "%s/PHOTOS/%s_%s_preview.jpg", base_, name,
               p.id);
      snprintf(side, sizeof(side), "%s/PHOTOS/%s_%s_preview.json", base_, name,
               p.id);
      FILE *f = fopen(tmp, "wb");
      if (f && fwrite(encoded_, 1, size, f) == size && fflush(f) == 0) {
        ...
        if (j) {
          ...
          fprintf(j, ",\n  \"width\":320,\n  \"height\":240,\n  \"intensity\":%d,\n  \"sourceTimestampMs\":%lld,\n  "
                     "\"originalPreserved\":true,\n  \"cameraModel\":", recIntensity_, (long long)saveTimestamp_);
          retrolens::jsonEscape(j, model_);
          fputs("\n}\n", j);
          fclose(j);
        }
        log("processed derivative saved path=%s bytes=%lu", path,
            (unsigned long)size);
      } else {
        if (f)
          fclose(f);
  ...
  void rect(uint16_t *bits, int stride, int x, int y, int w, int h,
    ...
    for (int yy = y; yy < y + h; yy++)
      for (int xx = x; xx < x + w; xx++)
        bits[yy * stride + xx] =
            alpha >= 100 ? color
                         : blend565(bits[yy * stride + xx], color, alpha);
  }
  void text(uint16_t *bits, int stride, int x, int y, const char *value,
  ...
  void render() {
    ...
    if (touchActive_ && now - touchDownMs_ > 450)
      compare_ = true;
    rect(bits, stride, 0, 0, kSurfaceWidth, kSurfaceHeight, rgb565(15, 18, 20),
         100);
    if (hasFrame_) {
    ...
    if (error_) {
      ...
      text(bits, stride, 130, 205, "ANALYTICAL LIVE VIEW COULD NOT START", warm,
           1);
      text(bits, stride, 150, 235, "NORMAL CAPTURE REMAINS AVAILABLE", accent,
           1);
    ...
    if (hasFrame_ && controlsVisible_) {
      ...
      char top[160];
      snprintf(top, sizeof(top), "%s   T%d   %d FPS   PHOTO", p.name, p.tier,
               fps_);
      ...
      for (int card = -1; card <= 1; card++) {
        int index = (selected_ + card + retrolens::presetCount()) %
                    retrolens::presetCount();
        ...
        text(bits, stride, x + 10, 399, retrolens::presetAt(index).category,
             accent, 1);
        text(bits, stride, x + 10, 425, retrolens::presetAt(index).name, warm,
             1);
        char tier[16];
        snprintf(tier, sizeof(tier), "TIER %d",
                 retrolens::presetAt(index).tier);
    ...
    if (scene_ == SCENE_DRAWER) {
      ...
      text(bits, stride, 60, 230, "FILES ARE STORED UNDER RETROLENS", accent,
           1);
...
Java_io_pihda_retrolens_NativeBridge_nativeCreate(JNIEnv *env, jclass,
                                                  jstring root, jstring model,
                                                  jstring version) {
...
Java_io_pihda_retrolens_NativeBridge_nativeDestroy(JNIEnv *, jclass, jlong h) {
  delete from(h);
}
extern "C" JNIEXPORT void JNICALL
Java_io_pihda_retrolens_NativeBridge_nativeSetSurface(JNIEnv *env, jclass,
                                                      jlong h,
                                                      jobject surface) {
  Runtime *r = from(h);
  if (r && surface)
...
extern "C" JNIEXPORT void JNICALL
Java_io_pihda_retrolens_NativeBridge_nativeClearSurface(JNIEnv *, jclass,
                                                        jlong h) {
  Runtime *r = from(h);
  if (r)
    r->clearSurface();
...
Java_io_pihda_retrolens_NativeBridge_nativeSubmitFrame(JNIEnv *env, jclass,
                                                       jlong h, jobject buffer,
                                                       jint length,
                                                       jlong timestamp) {
  Runtime *r = from(h);
...
extern "C" JNIEXPORT void JNICALL
Java_io_pihda_retrolens_NativeBridge_nativeSetPreviewError(JNIEnv *env, jclass,
                                                           jlong h,
                                                           jstring message) {
  Runtime *r = from(h);
  if (!r)
...
extern "C" JNIEXPORT void JNICALL
Java_io_pihda_retrolens_NativeBridge_nativeKey(JNIEnv *, jclass, jlong h,
...
extern "C" JNIEXPORT void JNICALL
Java_io_pihda_retrolens_NativeBridge_nativeTouch(JNIEnv *, jclass, jlong h,
...
extern "C" JNIEXPORT void JNICALL
Java_io_pihda_retrolens_NativeBridge_nativeCaptureRequested(JNIEnv *, jclass,
                                                            jlong h, jlong ts) {
  Runtime *r = from(h);
  if (r)
...
extern "C" JNIEXPORT void JNICALL
Java_io_pihda_retrolens_NativeBridge_nativeSaveSnapshot(JNIEnv *, jclass,
                                                        jlong h, jlong ts) {
  Runtime *r = from(h);
  if (r)
    r->saveSnapshot(ts);
...
extern "C" JNIEXPORT void JNICALL
Java_io_pihda_retrolens_NativeBridge_nativeToggleRecording(JNIEnv *, jclass,
                                                           jlong h, jlong ts) {
  Runtime *r = from(h);
  if (r)
...
extern "C" JNIEXPORT void JNICALL
Java_io_pihda_retrolens_NativeBridge_nativeStopRecording(JNIEnv *, jclass,
                                                         jlong h,
