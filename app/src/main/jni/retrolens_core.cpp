...
#define P(ID, NAME, CAT, DESC, TIER, FLAGS, CTRLS, R, G, B, SAT, CON, POST, NOISE, PIX, TEMP)      \
    {ID, NAME, CAT, DESC, TIER, FLAGS, CTRLS, R, G, B, SAT, CON, POST, NOISE, PIX, TEMP}

static const Preset kPresets[] = {
    P("olive_pocket", "Olive Pocket", "Handheld",
      "Four-tone olive LCD with ordered dither and ghosting", 1,
      FX_MONO | FX_DITHER | FX_PIXELATE | FX_TEMPORAL, CP, 72, 95, 48, -100, 15, 4, 3, 3, 18),
    P("silver_pocket", "Silver Pocket", "Handheld", "Cool reflective four-shade pocket display", 1,
    ...
    P("dot_matrix_toy", "Dot Matrix Toy", "Handheld", "Slow low-resolution dot-matrix toy camera",
      2, FX_MONO | FX_PIXELATE | FX_MASK | FX_TEMPORAL, CP | CTRL_MOTION, 70, 86, 58, -100, 20, 3,
      2, 5, 28),
    P("calculator_vision", "Calculator Vision", "Handheld",
      "Hard dark segments on pale green-gray LCD", 1, FX_MONO | FX_PIXELATE | FX_DITHER, CP, 58, 70,
      50, -100, 42, 2, 1, 5, 0),
    P("pocket_color", "Pocket Color", "Handheld", "Pastel limited-color handheld LCD", 1,
      FX_DITHER | FX_PIXELATE | FX_MASK, CP, 100, 102, 92, -12, 10, 5, 2, 3, 0),
    P("one_bit_desktop", "One-Bit Desktop", "Computer",
      "Pure one-bit desktop with square Bayer pixels", 1, FX_MONO | FX_DITHER | FX_PIXELATE, CP,
      100, 100, 100, -100, 55, 2, 0, 2, 0),
    P("cga_shock", "CGA Shock", "Computer", "Cyan-magenta-black-white computer palette", 1,
      FX_CGA | FX_PIXELATE | FX_DITHER, CP, 100, 100, 100, 25, 30, 4, 0, 4, 0),
    P("ega_sixteen", "EGA Sixteen", "Computer", "Hard sixteen-color early computer quantization", 1,
      FX_DITHER | FX_PIXELATE, CP, 105, 100, 105, 20, 22, 4, 0, 3, 0),
    P("vga_sixteen", "VGA Sixteen", "Computer", "Saturated VGA-inspired sixteen-color dither", 1,
      FX_DITHER | FX_PIXELATE, CP, 110, 105, 115, 42, 20, 4, 1, 2, 0),
    P("amber_terminal", "Amber Terminal", "Computer",
      "Amber phosphor terminal with character texture", 1,
      FX_MONO | FX_SCANLINES | FX_BLOOM | FX_ASCII, CP, 130, 76, 12, -100, 28, 5, 2, 4, 0),
    P("green_terminal", "Green Terminal", "Computer",
      "Green phosphor persistence and rolling refresh", 2,
      FX_MONO | FX_SCANLINES | FX_TEMPORAL | FX_BLOOM, CM, 25, 125, 48, -100, 25, 6, 2, 2, 20),
    P("teletext", "Teletext", "Computer", "Primary-color block graphics and character cells", 2,
      FX_CGA | FX_PIXELATE | FX_ASCII, CP, 105, 105, 105, 35, 28, 4, 0, 6, 0),
    P("ansi_camera", "ANSI Camera", "Computer", "ANSI-like colored text-cell shading", 2,
      FX_ASCII | FX_PIXELATE | FX_DITHER, CP, 108, 104, 100, 25, 25, 6, 1, 5, 0),
    P("ascii_mono", "ASCII Mono", "Computer", "Dense monochrome luminance characters", 3,
      FX_ASCII | FX_MONO | FX_PIXELATE, CP, 105, 96, 76, -100, 34, 6, 0, 5, 0),
    P("ascii_color", "ASCII Color", "Computer", "Colored character cells with adjustable density",
      3, FX_ASCII | FX_PIXELATE, CP, 105, 105, 105, 15, 22, 6, 0, 5, 0),
    P("braille_mosaic", "Braille Mosaic", "Computer", "Duotone dot-cell luminance mosaic", 2,
      FX_MONO | FX_HALFTONE | FX_PIXELATE, CP, 92, 104, 82, -100, 30, 3, 0, 4, 0),
    P("eight_bit_summer", "Eight-Bit Summer", "Console",
      "Bright chunky palette and checkerboard dither", 1, FX_DITHER | FX_PIXELATE, CP, 115, 108, 92,
      38, 18, 5, 1, 4, 0),
    P("sixteen_bit_arcade", "Sixteen-Bit Arcade", "Console",
      "Wide saturated palette with pixel sharpening", 1, FX_PIXELATE | FX_EDGE, CP, 110, 105, 110,
      42, 25, 6, 0, 2, 0),
    P("arcade_cabinet", "Arcade Cabinet", "Console", "Curved cabinet CRT, mask, and scanlines", 2,
      FX_PIXELATE | FX_SCANLINES | FX_MASK | FX_VIGNETTE, CV | CTRL_PIXEL, 105, 100, 104, 18, 24, 7,
      1, 3, 0),
    P("composite_console", "Composite Console", "Console",
      "Soft composite bleed, fringe, and dot crawl", 2, FX_TEAR | FX_SCANLINES | FX_TEMPORAL, CM,
      108, 96, 105, 15, 8, 8, 3, 1, 12),
    P("rf_channel_three", "RF Channel Three", "Console",
      "Unstable RF ghosts, snow, and sync errors", 2,
      FX_TEAR | FX_JITTER | FX_TEMPORAL | FX_SCANLINES, CM, 98, 98, 98, -10, 20, 7, 18, 1, 18),
    P("consumer_crt", "Consumer CRT", "Analog TV",
      "Soft bloom, scanlines, curvature, and convergence", 2,
      FX_SCANLINES | FX_MASK | FX_VIGNETTE | FX_BLOOM, CV, 105, 100, 103, 12, 16, 8, 2, 1, 0),
    P("aperture_grille", "Aperture Grille", "Analog TV",
      "Crisp vertical phosphor grille with mild bloom", 2, FX_SCANLINES | FX_MASK | FX_BLOOM, CV,
      106, 102, 105, 14, 20, 8, 1, 1, 0),
    P("shadow_mask", "Shadow Mask", "Analog TV", "Soft phosphor triads and darkened corners", 2,
      FX_MASK | FX_SCANLINES | FX_VIGNETTE, CV, 101, 98, 100, 5, 10, 8, 1, 1, 0),
    P("late_night_broadcast", "Late-Night Broadcast", "Analog TV",
      "Crushed late-night broadcast color and noise", 1, FX_SCANLINES | FX_VIGNETTE, CV, 110, 98,
      106, 35, 32, 8, 7, 1, 0),
    P("ntsc_composite", "NTSC Composite", "Analog TV",
      "Phase errors, horizontal bleed, and dot crawl", 2, FX_TEAR | FX_SCANLINES | FX_TEMPORAL, CM,
      108, 96, 104, 18, 12, 8, 4, 1, 10),
    P("pal_ghost", "PAL Ghost", "Analog TV", "Alternate-line color and restrained frame ghosting",
      2, FX_TEMPORAL | FX_SCANLINES | FX_TEAR, CM, 98, 104, 104, 4, 8, 8, 2, 1, 22),
    P("secam_state_tv", "SECAM State TV", "Analog TV",
      "Smeared state broadcast color in a 4:3 frame", 2, FX_TEAR | FX_SCANLINES | FX_VIGNETTE, CV,
      102, 98, 92, -25, 18, 7, 6, 1, 0),
    P("vhs_rental", "VHS Rental", "Tape", "Rental tape jitter, bleed, tracking, and head noise", 2,
      FX_TEAR | FX_JITTER | FX_TEMPORAL | FX_SCANLINES, CM, 108, 98, 90, -8, 12, 8, 10, 1, 18),
    P("damaged_vhs", "Damaged VHS", "Tape", "Heavy dropouts, tears, unstable tracking, and fading",
      2, FX_TEAR | FX_JITTER | FX_TEMPORAL | FX_SCANLINES, CM, 102, 92, 90, -28, 25, 7, 22, 1, 25),
    P("video8_family", "Video8 Family", "Tape", "Warm soft consumer video with exposure breathing",
      2, FX_TEMPORAL | FX_BLOOM | FX_VIGNETTE, CM, 112, 103, 90, 8, 5, 8, 7, 1, 13),
    P("hi8_vacation", "Hi8 Vacation", "Tape", "Bright sharp analog vacation tape", 2,
      FX_JITTER | FX_SCANLINES | FX_EDGE, CM, 112, 108, 96, 20, 18, 8, 5, 1, 0),
    P("minidv_2002", "MiniDV 2002", "Tape", "Cool clipped digital camcorder sharpness", 1,
      FX_EDGE | FX_MASK, CP, 96, 104, 114, 5, 30, 8, 3, 2, 0),
    P("early_webcam", "Early Webcam", "Tape", "Low-cadence block compression and smeared motion", 2,
      FX_PIXELATE | FX_TEMPORAL | FX_MASK, CM | CTRL_PIXEL, 105, 96, 110, 5, 25, 5, 7, 6, 30),
    P("cctv_1996", "CCTV 1996", "Tape", "Monochrome interlaced security monitor", 2,
      FX_MONO | FX_SCANLINES | FX_TEMPORAL, CM, 100, 103, 98, -100, 28, 6, 8, 1, 10),
    P("night_security", "Night Security", "Tape",
      "Simulated green night surveillance amplification", 2,
      FX_MONO | FX_SCANLINES | FX_VIGNETTE | FX_BLOOM, CV, 28, 124, 42, -100, 22, 6, 20, 1, 0),
    P("super_eight", "Super Eight", "Film", "Warm grain, gate weave, dust, and flicker", 2,
      FX_JITTER | FX_VIGNETTE | FX_BLOOM | FX_TEMPORAL, CM | CTRL_VIGNETTE, 114, 103, 84, 8, 12, 8,
      12, 1, 9),
    P("sixteen_millimeter_news", "Sixteen-Millimeter News", "Film",
      "High-contrast grain, scratches, and frame jitter", 2,
      FX_MONO | FX_JITTER | FX_EDGE | FX_VIGNETTE, CM | CTRL_VIGNETTE, 104, 100, 86, -80, 34, 7, 16,
      1, 0),
    P("soviet_archive_1978", "Soviet Archive 1978", "Archive",
      "Cinematic cadence, weave, dust, scratches, and splice flash", 2,
      FX_MONO | FX_JITTER | FX_TEMPORAL | FX_VIGNETTE | FX_TEAR, CM | CTRL_VIGNETTE, 94, 105, 78,
      -75, 24, 6, 18, 1, 22),
    P("state_archive_color", "State Archive Color", "Archive",
      "Faded uneven archival color with film jitter", 2, FX_JITTER | FX_TEMPORAL | FX_VIGNETTE,
      CM | CTRL_VIGNETTE, 108, 101, 88, -35, 0, 7, 9, 1, 12),
    P("cosmonaut_tape", "Cosmonaut Tape", "Archive",
      "Blue-gray telemetry tape with periodic signal loss", 2,
      FX_MONO | FX_TEAR | FX_SCANLINES | FX_MASK, CM, 72, 92, 110, -70, 22, 6, 14, 1, 0),
    P("expired_slide", "Expired Slide", "Film", "Shifted dye, lifted blacks, fading, and vignette",
      1, FX_VIGNETTE, CV, 116, 94, 112, -12, -5, 8, 5, 1, 0),
    P("bleach_bypass", "Bleach Bypass", "Film", "Metallic low-saturation contrast and grain", 1,
      FX_EDGE | FX_VIGNETTE, CV, 105, 102, 96, -65, 42, 8, 9, 1, 0),
    P("cross_process", "Cross Process", "Film", "Cyan shadows, yellow highlights, and hard curves",
      1, FX_VIGNETTE, CV, 104, 112, 88, 24, 38, 8, 4, 1, 0),
    P("orthochromatic", "Orthochromatic", "Film", "Old monochrome response with dark reds", 1,
      FX_MONO | FX_DITHER | FX_VIGNETTE, CV, 72, 110, 96, -100, 26, 7, 8, 1, 0),
    P("cyanotype", "Cyanotype", "Print", "Deep blue paper print with pale cyan highlights", 1,
    ...
    P("cmyk_misprint", "CMYK Misprint", "Print", "Offset halftone channels on tinted paper", 2,
      FX_HALFTONE | FX_TEAR | FX_DITHER, CP, 106, 98, 92, 18, 18, 5, 4, 2, 0),
    P("risograph", "Risograph", "Print", "Limited grainy inks with imperfect registration", 2,
      FX_CGA | FX_HALFTONE | FX_TEAR, CP, 108, 92, 96, 8, 25, 4, 8, 2, 0),
    P("manga_screen", "Manga Screen", "Print", "Black contours and screentone paper shading", 2,
      FX_MONO | FX_EDGE | FX_HALFTONE | FX_INVERT, CP, 100, 100, 100, -100, 44, 3, 2, 2, 0),
    P("photocopier", "Photocopier", "Print", "Hard dirty toner threshold and glass streaks", 1,
      FX_MONO | FX_EDGE | FX_DITHER, CP, 100, 100, 98, -100, 62, 2, 12, 1, 0),
    P("thermal_receipt", "Thermal Receipt", "Print", "Faded thermal marks and transport bands", 1,
      FX_MONO | FX_DITHER | FX_TEAR, CP, 96, 92, 82, -100, 38, 2, 7, 2, 0),
    P("pencil_draft", "Pencil Draft", "Print", "Edge-aware grayscale pencil shading", 3,
      FX_MONO | FX_EDGE | FX_INVERT | FX_DITHER, CP, 100, 100, 100, -100, 18, 6, 4, 1, 0),
    P("comic_ink", "Comic Ink", "Print", "Posterized regions with strong dark contours", 3,
      FX_EDGE | FX_DITHER, CP, 108, 103, 98, 18, 34, 4, 2, 2, 0),
    P("jpeg_hell", "JPEG Hell", "Digital Decay",
      "Repeated-generation blocks, ringing, and chroma loss", 3, FX_PIXELATE | FX_MASK | FX_TEAR,
      CP, 108, 94, 106, -12, 36, 4, 10, 8, 0),
    P("databent_preview", "Databent Preview", "Digital Decay",
      "Deterministic displaced blocks and frozen planes", 3,
      FX_TEAR | FX_JITTER | FX_MASK | FX_TEMPORAL, CM, 112, 92, 108, 22, 25, 6, 8, 4, 35),
    P("frame_echo", "Frame Echo", "Digital Decay", "Tinted temporal trails with controlled decay",
      2, FX_TEMPORAL | FX_TEAR, CM, 110, 98, 108, 12, 10, 8, 2, 1, 48),
    P("signal_tear", "Signal Tear", "Digital Decay", "Intermittent row splits and chromatic offset",
      2, FX_TEAR | FX_JITTER, CM, 110, 96, 108, 18, 24, 7, 8, 1, 0),
    P("pixel_sort", "Pixel Sort", "Digital Decay",
      "Thresholded row sorting in capture-quality mode", 3, FX_TEAR | FX_PIXELATE | FX_EDGE, CP,
      105, 102, 108, 20, 30, 6, 2, 3, 0),
    ...
    P("brown_apocalypse", "Brown Apocalypse", "Game Era",
      "Harsh brown-gray contrast, bloom, and dirt", 1, FX_VIGNETTE | FX_BLOOM, CV, 108, 82, 58, -48,
      42, 7, 9, 1, 0),
    P("teal_orange_blockbuster", "Teal-Orange Blockbuster", "Game Era",
      "Teal shadows and orange highlights with widescreen punch", 1, FX_VIGNETTE, CV, 112, 101, 96,
      28, 38, 8, 3, 1, 0),
    P("green_code", "Green Code", "Game Era", "Green-biased shadows and restrained digital noise",
      1, FX_VIGNETTE, CV, 78, 112, 82, -20, 36, 7, 5, 1, 0),
    P("blue_scifi_2004", "Blue Sci-Fi 2004", "Game Era",
      "Metallic blue shadows, cyan highlights, and bloom", 1, FX_BLOOM | FX_VIGNETTE, CV, 86, 104,
      120, -8, 32, 8, 4, 1, 0),
    P("bloom_shooter", "Bloom Shooter", "Game Era", "Overexposed desaturated bloom and sharpening",
      2, FX_BLOOM | FX_EDGE | FX_VIGNETTE, CV, 112, 108, 102, -35, 18, 8, 4, 1, 0),
    P("thermal_false_color", "Thermal False Color", "Experimental",
      "Simulated luminance thermal palette; not thermal sensing", 1, FX_THERMAL, CP, 100, 100, 100,
      0, 24, 8, 2, 1, 0),
    P("infrared_false_color", "Infrared False Color", "Experimental",
      "Simulated false infrared through channel remapping", 1, FX_INVERT | FX_VIGNETTE, CV, 72, 120,
      105, 35, 25, 8, 3, 1, 0),
    P("edge_scanner", "Edge Scanner", "Experimental", "Animated targeting sweep over an edge map",
      2, FX_EDGE | FX_MONO | FX_SCANLINES | FX_INVERT, CM, 30, 116, 104, -100, 45, 5, 3, 1, 0),
    P("blueprint", "Blueprint", "Experimental", "White technical edges on a measured blue grid", 2,
      FX_EDGE | FX_MONO | FX_INVERT | FX_MASK, CP, 25, 78, 120, -100, 36, 5, 1, 1, 0)};
...
void processFrame(const Pixel* source, Pixel* output, Pixel* scratch, const Pixel* previous,
    ...
    const int pixel =
        (preset.flags & FX_PIXELATE) ? (preset.pixelSize < 2 ? 2 : preset.pixelSize) : 1;
    for (int y = 0; y < height; ++y) {
        ...
        for (int x = 0; x < width; ++x) {
            ...
            if (preset.flags & FX_EDGE) {
                ...
                int edge = ((luma(source[y * width + xr]) - luma(source[y * width + xl])));
                if (edge < 0)
                    edge = -edge;
...
AviWriter::AviWriter()
    : file_(0), width_(0), height_(0), fps_(0), frameCount_(0), bytesWritten_(0),
      riffSizeOffset_(0), moviSizeOffset_(0), moviDataOffset_(0), avihFramesOffset_(0),
      strhFramesOffset_(0) {}
AviWriter::~AviWriter() {
    abort();
}
...
void AviWriter::writeFourCC(const char* v) {
    fwrite(v, 1, 4, file_);
}
...
bool AviWriter::addFrame(const unsigned char* jpeg, size_t size) {
    if (!file_ || !jpeg || !size || frameCount_ >= kMaxAviFrames || size > 1024U * 1024U)
        return false;
