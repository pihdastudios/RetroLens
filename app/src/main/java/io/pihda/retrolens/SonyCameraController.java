...
public final class SonyCameraController implements SurfaceHolder.Callback {
    ...
    public void stop() {
        ...
        previewReady = false;
        if (camera != null) {
            try { camera.getNormalCamera().cancelAutoFocus(); }
            catch (Throwable t) { report("Cancel autofocus during release failed", t); }
            try { camera.cancelTakePicture(); }
            catch (Throwable t) { report("Cancel shutter during release failed", t); }
            try { camera.setJpegListener(null); }
            catch (Throwable t) { report("Remove JPEG listener failed", t); }
            try {
                camera.release();
                Logger.info("Camera: CameraEx released");
            } catch (Throwable t) { report("Camera release failed", t); }
        }
    }
    ...
    public void focus(boolean start) {
        ...
        try {
            if (start) camera.getNormalCamera().autoFocus(null);
            else camera.getNormalCamera().cancelAutoFocus();
            Logger.info("Camera: autofocus " + (start ? "requested" : "cancelled"));
        } catch (Throwable t) { report("Autofocus failed", t); }
    }
    ...
    public boolean capture() {
        ...
        try {
            ...
            handler.postDelayed(new Runnable() {
                @Override
                public void run() {
                    if (!active || generation != captureGeneration || cameraEx != camera) return;
                    releaseShutter();
                }
            }, 425L);
            return true;
    ...
    public void releaseShutter() {
        ...
        } catch (Throwable t) { report("Shutter release failed", t); }
    }

    private void startPreviewIfReady() {
        if (!active || !surfaceReady || previewReady || cameraEx == null) return;
        try {
        ...
        } catch (IOException e) { report("Preview display failed", e); }
        catch (Throwable t) { report("Preview start failed", t); }
    }
